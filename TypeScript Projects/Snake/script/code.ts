...
const againBtn = document.querySelector("#restart-btn") as HTMLElement;
...
const render = (): void => {
  ...
  snake.forEach((seg, i) => {
    ...
    if (i === 0) {
      el.className = 'snakeHead bg-green-500 rounded-sm border border-green-600';
    } else {
      el.className = 'snakeBody bg-green-400 rounded-sm border border-green-500';
    }
