...
class TicTacToeGame {
  ...
  private updateCurrentPlayerDisplay(): void {
    ...
    if (currentPlayerElement) {
      ...
      currentPlayerElement.className =
        "text-2xl font-semibold px-8 py-4 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 text-white transition-all duration-300 shadow-lg";
  ...
  private updateStats(): void {
    ...
    if (statsElements.length >= 3) {
      (statsElements[0] as HTMLElement).textContent = this.stats.X.toString();
      (statsElements[1] as HTMLElement).textContent = this.stats.O.toString();
      (statsElements[2] as HTMLElement).textContent =
        this.stats.draws.toString();
    }
  }
  ...
  private showGameResult(message: string): void {
    ...
    if (resultModal && resultMessage) {
      ...
      resultModal.classList.add("opacity-100", "visible");
  ...
  private hideGameResult(): void {
    ...
    if (resultModal) {
      ...
      resultModal.classList.remove("opacity-100", "visible");
    }
  }
